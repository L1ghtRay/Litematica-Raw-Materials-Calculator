...
export default function ItemDisplay({ item, text, img }: { item: string, text?: string, img: string }) {
    ...
    return (
        <Tooltip tooltip={<MCTooltipBox item_id={item} scale="text-[3cqw]" />} className="">
            <BoxDisplay 
                ...
                textClassName={`text-slate-50 text-shadow-gray-600 pointer-events-none text-shadow-[1px_1px_0px_#133f3f] font-[Minecraft] absolute top-1/2 ${text && parseFloat(text) >= 100 ? 'left-3/5' : 'left-3/4'} -translate-x-1/2`}
            />
        </Tooltip>
    );
}